...
def run_v18_35_logic():
    ...
    with st.sidebar:
        ...
        da_target_prod = target_prod - sa_prod + 50 # 버퍼
        ...
        if upload_mode == "파일 업로드":
            ...
            plab_10_bojang = int(start_bojang * 0.6) # 임의 비율
    ...
    with tab2:
        ...
        # 10시 이후 가중치
        weights = [0.11, 0.18, 0.15, 0.11, 0.16, 0.10, 0.10, 0.09] 
        total_weight = sum(weights)
        
        gap = da_target_total - start_total
        
        acc_list = [start_total]
        hourly_secure = [start_total] # 10시는 시작 자원 그대로
        
        current_acc = start_total
        for w in weights:
            # 시간당 확보량
            secure = int(gap * (w / total_weight))
            current_acc += secure
            
            acc_list.append(current_acc)
            hourly_secure.append(secure)
            
        # 마지막 18시 보정 (반올림 오차 등)
        acc_list[-1] = da_target_total
        hourly_secure[-1] = da_target_total - acc_list[-2]
        
        # 인당 배분 계산
        per_person = [round(x / active_member, 1) if active_member > 0 else 0 for x in acc_list]
        
        # 표 데이터프레임 생성
        df_goal_table = pd.DataFrame({
            "구분": ["누적자원", "인당배분", "시간당 확보"],
            **{h: [f"{a:,}", f"{p}", f"{s:,}"] for h, a, p, s in zip(hours, acc_list, per_person, hourly_secure)}
        })
