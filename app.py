...
def analyze_data(df, aff_to_bojang=False):
    ...
    total = res['bojang_cnt'] + res['prod_cnt']
    if total > 0: res['ratio_ba'] = res['bojang_cnt'] / total
...
# -----------------------------------------------------------
# MODE 1: V6.6 Legacy (UI 복원)
# -----------------------------------------------------------
def run_v6_6():
    st.title("📟 Legacy Mode (V6.6 수기 입력)")
    st.markdown("🛠️ **기존 방식의 수기 입력 모드입니다.**")
    ...
    fixed_msg = f"금일 {fixed_content}." if fixed_ad_type != "없음" else "금일 특이사항 없이..."

    # 16시 로직 (간단 ver)
    mul_16 = 1.099
    est_18_from_16 = int(current_total * mul_16)
    rem = max(150, min(est_18_from_16 - current_total, 350))
    msg_16 = "* 보장분석 자원 넉넉할 것으로..." if current_total + rem >= da_target_18 else "* 마감 전까지 배너광고 및 제휴..."

    # 명일
    tom_total = int(tom_member * 3.15) + (300 if tom_dawn_ad else 0)
    tom_da = tom_total - tom_sa_9
    ad_msg = "\n* 명일 새벽 고정광고..." if tom_dawn_ad else ""
...
[17시 기준]
총 자원 : {da_target_17:,}건 ({active_member}명, {da_target_17/active_member:.1f}건 배정 기준)
ㄴ 보장분석 : {int(da_target_17*0.12):,}건
ㄴ 상품 : {int(da_target_17*0.88):,}건

[18시 기준]
총 자원 : {da_target_18:,}건 ({active_member}명, {da_target_18/active_member:.1f}건 배정 기준)
ㄴ 보장분석 : {int(da_target_18*0.12):,}건
ㄴ 상품 : {int(da_target_18*0.88):,}건
...
* {fixed_msg}"""
    ...
    with tab2:
        ...
        report_1400 = f"""DA파트 금일 현황 (Legacy)
...
{msg_16}"""
        st.text_area("복사 텍스트:", report_1600, height=300)

    with tab4:
        st.subheader("🌙 명일 자원 수립")
        report_tomorrow = f"""명일 오전 9시 예상

- 9시 예상 : {tom_total}건
ㄴ 보장 : {int(tom_da*0.12)}건
ㄴ 상품 : {int(tom_da*0.88)}건

* 인당 {tom_total/tom_member:.1f}건 예상{ad_msg}"""
        st.text_area("복사 텍스트:", report_tomorrow, height=200)


# -----------------------------------------------------------
# MODE 2: V7.1 (V9.0 Update)
# -----------------------------------------------------------
def run_v7_1():
    st.title("📊 메리츠화재 DA 통합 시스템 (V9.0)")
    st.markdown("🚀 **자동화 & 제휴 CPA 역산 기능 탑재**")
    ...
    msg_14 = "금일 고정구좌 이슈없이 집행중이며..." if est_18_from_14 >= da_target_18 else "오전 목표 대비 소폭 부족할 것으로..."
    msg_16 = "* 보장분석 자원 넉넉할 것으로..." if current_total + remaining_gap >= da_target_18 else "* 마감 전까지 배너광고 및 제휴..."
    
    dash_view = pd.DataFrame()
    if uploaded_realtime and not real_data['media_stats'].empty:
        ...
        dash_view = d_raw[sorted(d_raw.columns.tolist())]
