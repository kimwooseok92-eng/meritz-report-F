...
[예상 마감 - 24시 기준]
- 총합(DA/제휴): {est_cost_24//10000:,}만원 / 가망CPA {max(2.9, cpa_total-0.4)}만원"""
...
# -----------------------------------------------------------
# MODE 2: V9.0 (New Advanced)
# -----------------------------------------------------------
def run_v9_0_advanced():
    st.title("📊 메리츠화재 DA 통합 시스템 (V9.0 Advanced)")
...
def main():
    ...
    version = st.sidebar.selectbox(
        "사용할 버전을 선택하세요:",
        ["V9.0 (최신/Advanced)", "V6.6 (Legacy/안정화)"]
    )
    
    if version == "V9.0 (최신/Advanced)":
        run_v9_0_advanced()
    else:
        run_v6_6_legacy()
