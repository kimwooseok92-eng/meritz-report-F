...
# 0. 공통 설정 & 폰트
# -----------------------------------------------------------
st.set_page_config(page_title="메리츠 보고 자동화 V8.1", layout="wide")
...
def analyze_data(df, aff_to_bojang=False):
    ...
    res['aff_cnt'] = int(df[mask_aff_camp]['count'].sum()) # 제휴 건수
    res['da_cnt'] = int(df[~mask_aff_camp]['count'].sum()) # DA 건수
...
# -----------------------------------------------------------
# MODE 1: V6.6 (안정화 버전 - 기존 로직 복원)
# -----------------------------------------------------------
def run_v6_6():
    st.title("📊 메리츠화재 DA 보고 자동화 (V6.6 Stable)")
    st.info("ℹ️ 기존에 사용하던 안정화 버전입니다.")

    with st.sidebar:
        st.header("1. 기본 설정")
        day_option = st.selectbox("오늘 요일", ['월', '화', '수', '목', '금'], index=0)
        target_mode = st.radio("목표 수립 기조", ['평시', '이슈/보수적', '월말/공격적'], index=1 if day_option=='월' else 0)
        ...
        if uploaded_realtime:
            def_total, def_bojang, def_prod = real_data['total_cnt'], real_data['bojang_cnt'], real_data['prod_cnt']
    ...
    report_1400 = f"""DA파트 금일 현황 (V6.6)
    
목표: {da_target_18}건 (인당 {da_per_18})
현황: {current_total}건
예상: {est_18_from_14}건
...
제휴 CPA: {cpa_aff}만원
통합 CPA: {cpa_total}만원"""
    st.text_area("복사 텍스트:", report_1400, height=300)
    
    # 대시보드 (V6.6의 핵심 - 16시 분리)
    dash_16 = pd.DataFrame()
    ...
    if not dash_16.empty:
        st.dataframe(dash_16.style.background_gradient(cmap='Greens').format("{:,}"))
    else:
        st.info("파일 업로드 시 대시보드가 표시됩니다.")


# -----------------------------------------------------------
# MODE 2: V7.1 (최신 기능 + 수기 입력 보완)
# -----------------------------------------------------------
def run_v7_1():
    st.title("📊 메리츠화재 DA 통합 시스템 (V7.1 Advanced)")
    st.markdown("🚀 **자동화 & 대시보드 & 수기보완 모드**입니다.")
    ...
    with st.sidebar:
        ...
        # [핵심 수정] 수기 입력 시 제휴 건수 입력 추가
        if uploaded_realtime and not real_data['media_stats'].empty:
            st.success(f"자동 집계 완료: {real_data['total_cnt']:,}건")
            def_total, def_bojang, def_prod = real_data['total_cnt'], real_data['bojang_cnt'], real_data['prod_cnt']
            def_cost_da = real_data['da_cost_only']
            def_aff_cnt = real_data['aff_cnt'] # 파일에서 읽은 제휴 건수
        else:
            def_total, def_bojang, def_prod = 1963, 1600, 363
            def_cost_da = 23560000
            def_aff_cnt = 0 # 기본값
        ...
        if not uploaded_realtime:
            st.info("✏️ **[수기 보완]** 정확한 CPA 계산을 위해 제휴 건수를 입력해주세요.")
            current_aff_cnt = st.number_input("제휴 실적 (건수)", value=def_aff_cnt, help="수기 입력 시 필수! 제휴 CPA 계산에 쓰입니다.")
        else:
            current_aff_cnt = def_aff_cnt # 파일 있으면 자동값 사용
    ...
    # --- 계산 로직 ---
    # DA 건수 역산 (전체 - 제휴)
    current_da_cnt = current_total - current_aff_cnt
    
    # CPA 정밀 계산 (수기 모드에서도 정확하게)
    ...
    if not real_data['media_stats'].empty:
        ...
        for col in d14.columns:
            if '현재' in col and '비용' not in col:
                ...
                d16[col.replace('현재', '예상')] = (d16[col] * mul_16).astype(int)
        dash_14 = d14[sorted(d14.columns.tolist())]
        dash_16 = d16[sorted(d16.columns.tolist())]
    ...
    with tab0:
        ...
        c1.metric("DA 전체 목표", f"{da_target_18:,}건")
        c2.metric("보장 목표", f"{da_target_bojang:,}건")
        c3.metric("상품 목표", f"{da_target_prod:,}건")
        st.markdown("---")
        
        if not dash_view.empty:
            st.dataframe(dash_view.style.format("{:,}").background_gradient(cmap='Blues'), use_container_width=True)
        ...
        ax.plot(hours, ba_flow, marker='o', label='보장', color='#d62728')
        ax.plot(hours, prod_flow, marker='s', label='상품', color='#1f77b4')
        ax.legend()
...
[현재 성과 - 14시 기준]
- 총합(DA/제휴): {int(cost_total)//10000:,}만원 / 가망CPA {cpa_total}만원
- DA: {int(cost_da)//10000:,}만원 / 가망CPA {cpa_da}만원
- 제휴: {int(cost_aff)//10000:,}만원 / 가망CPA {cpa_aff}만원

[예상 마감 - 18시 기준]
- 총합(DA/제휴): {int(cost_total * 1.35)//10000:,}만원 / 가망CPA {max(3.1, cpa_total-0.2)}만원
- DA: {int(cost_da * 1.4)//10000:,}만원 / 가망CPA {max(4.4, cpa_da)}만원
- 제휴: {int(cost_aff * 1.25)//10000:,}만원 / 가망CPA {max(2.4, cpa_aff-0.2)}만원"""
...
def main():
    ...
    version = st.sidebar.selectbox(
        "사용할 버전을 선택하세요:",
        ["V7.1 (최신/자동화/수기보완)", "V6.6 (Legacy/안정화)"]
    )
    
    if version == "V7.1 (최신/자동화/수기보완)":
        run_v7_1()
    else:
        run_v6_6()
