...
# 0. 공통 설정 & 폰트
# -----------------------------------------------------------
st.set_page_config(page_title="메리츠 보고 자동화 V11.1", layout="wide")
...
* {fixed_msg}{issue_text}"""
        ...
        # [표 수정] 깨지는 matplotlib 표 제거 -> 깔끔한 st.table로 교체
...
# -----------------------------------------------------------
# MODE 2: V7.1 (New Advanced - NameError 수정)
# -----------------------------------------------------------
def run_v7_1_advanced():
    ...
    if uploaded_realtime and not real_data['media_stats'].empty:
        ...
        # 여기서 dash_14, dash_16에 동일하게 할당하거나, 옵션에 따라 분기
        # 단순화를 위해 현재 선택된 시점 데이터를 dash_14에 넣어서 14시 탭에 표시
        dash_14 = d_raw[sorted(d_raw.columns.tolist())]
        dash_16 = dash_14 # 16시도 동일 데이터 (시점 선택에 따라 값은 이미 위에서 계산됨)
